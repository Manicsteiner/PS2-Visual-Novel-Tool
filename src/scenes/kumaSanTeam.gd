...
#TODO: Huffman decompression for Aoi Sora no Neosphere - Nanoca Flanka Hatsumei Koubouki 2.
# See lzh_aoi.gd

func _ready() -> void:
	file_load_exe.filters = ["SLPS_255.14"]
	file_load_bin.filters = ["*.PAC, ASN.DAT"]
...
func extract_pac() -> void:
	for file: int in selected_pacs.size():
		...
		if selected_pacs[file].get_file() == "ASN.DAT":
			var num_files: int = in_file.get_32()
			
			var dir: DirAccess = DirAccess.open(folder_path)
			dir.make_dir_recursive(arc_name)
			
			var tbl_start: int = 0x10
			for i in range(num_files):
				var pos: int = (i * 0x2C) + 16
				in_file.seek(pos)
				
				var f_name: String = in_file.get_buffer(0x20).get_string_from_ascii()
			...
			if not selected_exe:
				OS.alert("Please load an exe first (SLPS_xxx.xx) for file %s" % arc_name)
				return
