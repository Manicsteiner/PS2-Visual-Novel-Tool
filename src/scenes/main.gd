...
func _on_game_type_selector_item_selected(index: int) -> void:
	if index == THREELDK:
		...
		game_type_sub_text.text = "Supports:\nExtraction, images (PNG)."
		game_type_selector.select(GOKUJOU)
		game_type = GOKUJOU
		var next_scene: PackedScene = load("res://src/scenes/Kid.tscn")
		sceneChanger(next_scene)
		...
		var next_scene: PackedScene = load("res://src/scenes/ZeroSystem.tscn")
		sceneChanger(next_scene)
	elif index == SHIRACHUU:
		game_type_sub_text.text = "Supports:\nExtraction, images (TM2, PNG)."
		game_type_selector.select(SHIRACHUU)
		game_type = SHIRACHUU
		var next_scene: PackedScene = load("res://src/scenes/NexEntertainment.tscn")
		sceneChanger(next_scene)
		...
		sceneChanger(next_scene)
	elif index == SAKURATAISEN5:
		game_type_sub_text.text = "Supports:\nExtraction (CVM to ISO only)."
		game_type_selector.select(SAKURATAISEN5)
		...
		game_type_sub_text.text = "Supports:\nExtraction, images (PNG)."
		game_type_selector.select(WIND)
		game_type = WIND
		...
		game_type_sub_text.text = "Supports:\nExtraction, images (PNG).\nUnknown swizzled image data."
		game_type_selector.select(IZAYOI)
		game_type = IZAYOI
		var next_scene: PackedScene = load("res://src/scenes/HuneX.tscn")
		sceneChanger(next_scene)
	elif index == KOMOREBI:
...
func initMenuItems() -> void:
	...
	game_type_selector.add_item("Gokujou Seitokai", GOKUJOU)
	...
	game_type_selector.add_item("Izayoi Renka: Kamifurusato", IZAYOI)
	...
	game_type_selector.add_item("Sakura Wars: So Long, My Love", SAKURATAISEN5)
	...
	game_type_selector.add_item("Shirachuu Tankenbu", SHIRACHUU)
	...
	game_type_selector.add_item("Wind: A Breath of Heart", WIND)
