			...
			for files in range(num_files - 1):
				...
				if f_ext == "KLZ":
					...
					if buff.slice(0, 4).get_string_from_ascii() == "TIM2":
						...
						print("%08X %08X %08X %s/%s" % [f_offset, f_size, buff.size(), folder_path, f_name])
						continue
					else:
						f_name += ".BIN"
...
func lzh_decode(input: PackedByteArray) -> PackedByteArray:
	var full_out: PackedByteArray
	var chunk: PackedByteArray
	chunk.resize(0x4000)  # working window

	# === Header ===
	var total_expected: int = ComFuncs.swap32(input.decode_u32(0))     # original: output_size
	var first_block_len: int = ComFuncs.swapNumber(input.decode_u16(4), "16")  # original: fill_count

	# === State mirrored 1:1 with original ===
	var out_pos: int = 0                 # original: s0 (write index into chunk)
	var src_index: int = 0               # original: s1 (copy source index)
	var seq_word: int = 0                # original: s2 (packed len/offset)
	var remain: int = 0                  # original: s3 (bytes remaining to copy in backref)
	var bit_index: int = 0               # original: OO40_sp (0..7 which bit to test)
	var bytes_in_block: int = 0          # original: OO42_sp (input bytes consumed within block)
	var control_ptr: int = 0             # original: OO44_sp (current control byte position)
	var data_ptr: int = 0                # original: OO48_sp (next data byte position)
	var out_base: int = 0                # original: OO50_sp (always 0; base of chunk)
	var start_ptr: int = 0               # original: OO60_sp
	var block_len: int = first_block_len # original: OO70_sp (current block length)
	var next_read_pos: int = 0
	var total_written: int = 0           # original: count (accumulated produced bytes)
	var pass_count: int = 0

	# === Control-bit decode table (unchanged) ===
	var mask_table: PackedByteArray = [
		0x01, 0x02, 0x04, 0x08,
		0x10, 0x20, 0x40, 0x80,
		# only first 8 used; rest left intact to match original
		0x81, 0x75, 0x81, 0x69,
		0x00, 0x00, 0x00, 0x00,
		0x01, 0x02, 0x00, 0x00
	]

	# === Pre-pass for big raw blocks (> 0x4000) ===
	chunk.resize(0x4000)
	if block_len > 0x4000:
		next_read_pos = 4
		while block_len > 0x4000:
			var cnt: int = 0
			var copy_off: int = next_read_pos + 2
			while cnt < 0x4000:
				full_out.append(input.decode_u8(copy_off))
				cnt += 1
				copy_off += 1
			total_written += 0x4000
			next_read_pos += cnt + 2
			pass_count += 1
			block_len = ComFuncs.swapNumber(input.decode_u16(next_read_pos), "16")
			if total_written >= total_expected or next_read_pos >= input.size():
				return full_out
		start_ptr = next_read_pos + 2
	else:
		start_ptr = 6

	# Initialize control/data pointers 
	control_ptr = start_ptr
	data_ptr = start_ptr + 1

	while true:
		# Read control byte and select current bit mask
		var control: int = input.decode_u8(control_ptr)
		var mask: int = mask_table[bit_index & 0xFF]

		if (control & mask) == 0:
			# --- Literal path  ---
			var lit: int = input.decode_u8(data_ptr)
			chunk.encode_s8(out_base + out_pos, lit)
			data_ptr += 1
			bytes_in_block += 1
			out_pos += 1
		else:
			# --- Backref path  ---
			bytes_in_block += 2
			var hi: int = input.decode_u8(data_ptr)
			var lo: int = input.decode_u8(data_ptr + 1)
			seq_word = ((hi << 8) | lo) & 0xFFFF

			remain = ((seq_word & 0x1F) + 2) & 0xFFFF
			src_index = ((out_pos - ((seq_word >> 5) & 0xFFFF) - 1) & 0xFFFF)

			# The original increments data_ptr as two separate +1 operations:
			# first before the loop, then once more after the loop.
			data_ptr += 1
			while true:
				# Early-window quirk: if out_pos < 0x0800 and out_pos < src_index,
				# copy from base index 0 (out_base), not from src_index.
				var use_base_zero: bool = (out_pos < 0x0800) and (out_pos < (src_index & 0xFFFF))
				var val: int
				if use_base_zero:
					val = chunk.decode_u8(out_base)
				else:
					val = chunk.decode_u8(out_base + (src_index & 0xFFFF))
				chunk.encode_s8(out_base + out_pos, val)
				out_pos += 1
				src_index = (src_index + 1) & 0xFFFF

				var prev_remain: int = remain
				remain = (remain - 1) & 0xFFFF
				if prev_remain == 0:
					break
			data_ptr += 1  # second +1, total +2 for a backref token

		# Advance bit; after 8 bits, the next control byte is at current data_ptr
		bit_index += 1
		if bit_index == 8:
			bit_index = 0
			control_ptr = data_ptr
			data_ptr += 1
			bytes_in_block += 1

		# Block boundary check: triggers when bytes_in_block >= (block_len - 1)
		var lhs: int = bytes_in_block & 0xFFFF
		var rhs_limit: int = (block_len - 1)
		var end_block: bool = not (lhs < rhs_limit)
		if end_block:
			# Update total produced count using the original bookkeeping
			total_written += out_pos

			# Append the whole 0x4000 working window
			full_out.append_array(chunk)
			if total_written >= total_expected or next_read_pos >= input.size():
				return full_out

			pass_count += 1
			if pass_count == 1:
				next_read_pos += block_len + 6
			else:
				next_read_pos += block_len + 2

			# Load next block length
			block_len = ComFuncs.swapNumber(input.decode_u16(next_read_pos), "16")

			# Raw-copy fast path for intervening >0x4000 blocks
			if block_len > 0x4000:
				while block_len > 0x4000:
					var cnt2: int = 0
					var copy_off2: int = next_read_pos + 2
					while cnt2 < 0x4000:
						full_out.append(input.decode_u8(copy_off2))
						cnt2 += 1
						copy_off2 += 1
					total_written += 0x4000
					next_read_pos += cnt2 + 2
					block_len = ComFuncs.swapNumber(input.decode_u16(next_read_pos), "16")
					if total_written >= total_expected or next_read_pos >= input.size():
						return full_out

			# Reset per-block state
			if data_ptr > input.size():
				return full_out
			out_pos = 0
			src_index = 0
			out_base = 0
			bytes_in_block = 0
			data_ptr = next_read_pos + 2
			bit_index = 0
			start_ptr = data_ptr
			control_ptr = start_ptr
			data_ptr = start_ptr + 1
